...
namespace OnlineTrading.Repository.Base.Implementations
{
    public abstract class BaseRepository<T> : IBasicFunctions<T> where T : class
    {
        public abstract string[] GetColumns();
        public abstract string GetTableName();
        public abstract T MapEntity(SqlDataReader reader);

        protected async Task<int> CreateAsync(T entity, string idDbFieldEnumeratorName = null)
        {
            ...
            command.CommandText = $@"INSERT INTO {GetTableName()} ({columns}) VALUES ({parameters}); SELECT CAST(SCOPE_IDENTITY() as int)";
            ...
            if(idDbFieldEnumeratorName != null)
        ...
        protected async Task<T> RetrieveAsync(string idDbFieldName, int idDbFieldValue)
        {
            using SqlConnection connection = await ConnectionFactory.GetOpenConnection();
        ...
        protected async IAsyncEnumerable<T> RetrieveCollectionAsync(Filter filter)
        {
            using SqlConnection connection = await ConnectionFactory.GetOpenConnection();
            using SqlCommand sqlCommand = connection.CreateCommand();
